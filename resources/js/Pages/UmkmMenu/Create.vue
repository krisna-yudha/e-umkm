...
const submit = () => {
    ...
    if (form.gambar_menu && form.gambar_menu instanceof File) {
        form.post(route('umkm.menu.store', props.umkm.id), {
            forceFormData: true
        });
    } else {
        // For regular submission without file, use normal POST
        form.post(route('umkm.menu.store', props.umkm.id));
    }
};
</script>
...
<template>
    <Head :title="`Tambah Menu - ${umkm.nama_umkm}`" />
            ...
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h2 class="text-xl font-semibold leading-tight text-gray-800">
                        Tambah Menu - {{ umkm.nama_umkm }}
                    </h2>
                    <p class="text-sm text-gray-600">Tambahkan produk/menu baru untuk UMKM Anda</p>
                </div>
            </div>
        </template>
                    ...
                    <form @submit.prevent="submit" class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <div 
                                ...
                                @drop="handleDrop"
                                :class="['border-2 border-dashed rounded-xl p-6 text-center transition-all duration-200 cursor-pointer', isDragOver ? 'border-purple-400 bg-purple-50' : 'border-gray-300 hover:border-purple-400']"
                            >
                                <input 
                                    ref="fileInput"
                                    type="file" 
